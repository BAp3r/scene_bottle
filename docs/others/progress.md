...
## 阶段七：离线 RGB 相机链路打通与亮度调优
**时间**: 2026-03-03
**状态**: ✅ 已跑通（持续优化中）

**工作内容**:
1. **脚本路径重构完成**：按新目录结构修复 `scripts` 下 USD 加载路径（`usds/objects/...`）。
2. **离线资产采图链路打通**：基于 `orbbec_gemini_335L.usd` + `Office/office.usd` 成功保存 RGB 图像。
3. **关键报错修复**：解决 `xformOp:orient` 的 `quatd/quatf` 精度冲突，避免流程在参考资产后中断。
4. **相机抓图能力增强**：
   * 支持 `--scene-usd` 指定离线场景；
   * 支持 `--camera-prim` 显式选择 RGB 相机；
   * 支持 `--camera-translate` / `--camera-orient` 快速调机位；
   * 支持 `--post-exposure` / `--post-gamma` 进行亮度后处理。
5. **结果确认**：
   * 已从“白图/空帧”进展为“可稳定抓到真实场景内容（当前偏向墙面且偏暗）”；
   * 说明离线传感器链路已建立，下一步重点是机位策略与自动曝光策略。

**产出文件**:
* `scripts/capture_rgb_offline.py`
* `outputs/rgb/orbbec_office_w240.png`
* `outputs/rgb/orbbec_office_bright.png`

## 下一步执行建议（已确认）
1. **随机机位模式（用于快速探索可见视角）**
   * 在限定的工作空间范围内随机采样相机位姿，批量抓图并按亮度/纹理信息筛选可用视角。
2. **GUI 指定位姿模式（用于人工精调）**
   * 使用 Isaac Sim Camera Inspector 在 GUI 中手动选定理想机位；
   * 将位姿回填到脚本参数（或配置文件），固定为后续数据采集标准机位。
